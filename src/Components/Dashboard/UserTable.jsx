...
import { useGetHostUserQuery } from '../../redux/Api/userApi';
import { imageUrl, placeImage } from '../../redux/Api/baseApi';

const UserTable = () => {
    const [page, setPage] = useState(1)
    const [search, setSearch] = useState('')
    const { data: getAllUser } = useGetHostUserQuery({ user: 'USER' , page : page , search : search})
        ...
        },
        {
            ...
            render: (text, record) => (
                <div className="flex items-center">
                    <img src={record.avatar} className='h-12 rounded-md' alt={record.name} />
                    <div className="ml-2">
                        <div>{record.name}</div>
    ...
    const formattedTableData = getAllUser?.data?.map((user, i) => {
        return {
            serialNo: i + 1,
            name: user?.name,
            avatar: user?.profile_image ? `${imageUrl}/${user?.profile_image}` : placeImage,
            email: user?.email,
    ...
    return (
        <div className=' rounded-md bg-white p-4'>
            <div className='between-center px-3 my-2 py-5 '>
                <p className='text-xl'>New User</p> <Input className='max-w-[250px] h-10' onChange={(e)=>setSearch(e.target.value)} prefix={<CiSearch className='text-2xl' />} placeholder="Search here..." />
            </div>
            <Table
                columns={columns}
